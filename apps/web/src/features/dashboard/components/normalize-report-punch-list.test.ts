...
test("reload of persisted Trust leftover punch reads stored JSON, not toast memory", () => {
  ...
  assert.equal(templateRowPunchLabel(reloaded), "1 leftovers");
  assert.ok(!templateRowPunchLabel(reloaded)?.includes("tagged"));
  assert.equal(leftoverPunchFromNormalizeReport(reloaded)[0]?.before, "do/do not");
  assert.equal(wordBlankDisplay(reloaded!.softSuggestions[0]!.before), "_[do/do not]_");
});
...
test("Needs attention drops already-tagged mapper loops", () => {
  const persisted = storedTrustReport({
    softSuggestions: [
      {
        id: "soft:0:blank_do_do_not",
      ...
      },
      {
        id: "soft:2:residuary",
        ruleId: "loop",
        before: "{#distribution_residuary}",
        rationale: "already tagged",
        applicable: false,
      },
    ],
    leftAsSuggestionCount: 3,
  });
  const leftovers = leftoverPunchFromNormalizeReport(persisted);
  assert.equal(leftovers.length, 1);
  assert.equal(wordBlankDisplay(leftovers[0]!.before), "_[do/do not]_");
  assert.ok(!leftovers.some((r) => r.before.includes("{#children}")));
  assert.ok(!leftovers.some((r) => r.before.includes("{#distribution_residuary}")));
});
...
test("accepted soft suggestions drop out of leftover punch after reload", () => {
  ...
  assert.equal(punch.punchLabel, "Ready for intake");
  assert.equal(templateRowLeftoverLabel(punch.report), "Ready for intake");
});
...
test("templateDisplayFileName prefers sourceFileName, else strips storage timestamp", () => {
  assert.equal(
    templateDisplayFileName("templates/firm/revocable_trust/Trust-_Family-lz8k3.docx", "Trust-_Family.docx"),
    "Trust-_Family.docx",
  );
  assert.equal(
    templateDisplayFileName("templates/firm/revocable_trust/Trust-_Family-lz8k3.docx"),
    "Trust-_Family.docx",
  );
});
