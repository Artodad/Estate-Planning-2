        ...
        <div className="rounded-lg border bg-card p-4">
          ...
          {lastPackage && (
            <div className="mt-4 rounded-md border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-800 dark:border-emerald-900/50 dark:bg-emerald-950/30 dark:text-emerald-200">
              ...
              {/* Wave F PDF guidance (text only) */}
              <p className="mt-1 text-[10px] text-emerald-700 dark:text-emerald-300">
                Need a PDF? Open the .docx in Microsoft Word (or LibreOffice) → File &gt; Save As / Export &gt; PDF. Guarantees 100% fidelity.
              </p>
            </div>
          )}
        </div>
...
export default async function ClientDetailPage({ params }: ClientDetailPageProps) {
  ...
  // Lightweight context is sufficient for the overview. Profile details can be added with { includeProfile: true } if needed for display.
  ...
  const allDocs = await generatedDocumentHelpers.listByFirm(client.firmId || "");
